...
describe('Migrations apply clean on a fresh DB', () => {
  it('all 9 tables have RLS ENABLED (Neon-compat: not FORCEd — owner bypass needed for SECURITY DEFINER auth)', async () => {
    ...
    >(
      `select relname, relrowsecurity, relforcerowsecurity
         from pg_class
        where relname in ('org','agent','task','decision','verdict_event','transition','score','membership','api_key')
          and relkind = 'r'
        order by relname`,
    );
  ...
  it('every table carries a tenant-isolation policy', async () => {
    ...
    expect(Number(rows[0]?.count)).toBe(9); // 7 original + membership (B) + api_key (C1)
  });
});
