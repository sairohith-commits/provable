...
// verified identity + API-resolved role collapses to the SAME AuthContext shape. The API/core
// never branch on providerType — they consume only orgId (+ subject/approver). Flipping
// AUTH_PROVIDER selects a different provider with zero downstream change.
const identity = { userId: 'u-1', email: 'a@b.com', displayName: 'Ada', emailVerified: true };
...
describe('AUTH_PROVIDER swaps the provider with no core/api change', () => {
  ...
  it.each<[AuthProviderType, string]>([
    ...
    ['oidc', 'org_workspace'],
    ['local', 'org_workspace'],
  ])('%s yields a canonical AuthContext (role carried, orgId carried)', (type, orgId) => {
    process.env['AUTH_PROVIDER'] = type;
    expect(activeProviderType()).toBe(type);

    const ctx = buildAuthContext(type, identity, orgId, 'APPROVER');
    expect(ctx).toEqual({
      userId: 'u-1',
      orgId,
      email: 'a@b.com',
